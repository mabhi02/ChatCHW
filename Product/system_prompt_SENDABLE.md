# CHW Navigator System Prompts (sendable)

_Frozen for the 2026-04-14 WHO 2012 run (`run_d9a39c2d`)._
_Model: Claude Opus 4.6 at temperature 0 for every stage._
_Source files: `backend/gen7/labeler.py` (Stage 1 + 2), `backend/gen7/pipeline.py` + `backend/system_prompt.py` (Stage 3)._

---

## Pipeline overview

The pipeline runs three Opus calls against the same content-hashed manual. The identification claim is that the system prompt is the only experimental variable: the chunking is deterministic, the codebook is frozen, the validators downstream are Python code. Any run-to-run variance is attributable to Opus's own stochasticity, not to orchestration.

| Stage | Phase | Prompt used | Input | Output |
|-------|-------|-------------|-------|--------|
| 1 | Per-chunk labeling | `_build_labeling_system_prompt` | one ~2K-token chunk | labels array (span, id, type, subtype, quote_context) |
| 2 | Label QC / distillation (optional) | `_build_distillation_system_prompt` | Stage 1 candidate labels | verified labels (same schema) |
| 3 | REPL compilation | `backend/system_prompt.py::build_system_prompt` | deduped labels + reconstructed guide | 7 DMN artifacts + XLSForm |

The same naming codebook is pasted into all three stages so IDs are compatible across stages.

---

## Stage 1 — Chunk labeling system prompt

Used once per micro-chunk (21 chunks for the WHO 2012 manual). Anthropic prompt caching is set on the system block so the codebook + instructions are read at cache-read rates ($0.50/M) for every chunk after the first.
...
NAMING CODEBOOK:
<<NAMING_CODEBOOK (see `Shared naming codebook` section below)>>
...
Optional second pass. Runs only when Stage 1's output is suspect (hallucinated spans, wrong prefixes). The QC pass NEVER adds new labels: it only verifies grounding and corrects or drops bad IDs.
...
This is the prompt that drove the 2026-04-14 WHO 2012 run end-to-end. Two cached blocks attach to this prompt at runtime (the full reconstructed guide text, and the deduped label inventory) so every REPL turn and every `llm_query_batched` sub-call sees them at cache-read rates.

The `{custom_tools_section}` placeholder at the end is filled in by the rlm library at runtime with the tool signatures (`emit_artifact`, `validate`, `z3_check`, `FINAL_VAR`, `llm_query`, `llm_query_batched`, `rlm_query_batched`).
...
- **Determinism levers.** Sorting, content-derived IDs, and explicit for-loops are called out in every stage. Random tie-breakers are banned. Stage 3 phase C (dispatcher assembly) is pure Python, no LLM call, so routing order is identical across runs.
- **Catchers are not prompts.** The seven phase validators (`backend/validators/`) are Python code that inspects each emitted artifact; they do not add LLM calls during the main run. Their pass/fail verdicts are what land in `final_dmn.validator.json`.
- **Codebook is dynamic.** If the manual covers a concept that does not fit any existing prefix, Stage 3 can extend the codebook with a new prefix and record it in the `variables` artifact's `codebook_extensions` field. The codebook shown above is the starter template.
