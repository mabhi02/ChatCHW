#!/usr/bin/env python3
"""
langgraph_medical_assessment.py

This file replicates the functionality of the original medical assessment chatbot,
but uses a graph-based (nodes/edges) architecture (simulating langgraph agents) rather
than an explicitly linear implementation.

Key features maintained:
  - Pinecone integration for vector search.
  - Groq LLM calls (wrapped in helper functions).
  - MATRIX pattern analysis.
  - Data structures (structured questions and examination history) are stored in
    the graph memory.
    
To run, set up your environment variables (GROQ_API_KEY, PINECONE_API_KEY, OPENAI_API_KEY)
...
# Import (or simulate) langgraph agent components
# In an actual implementation, you might import:
# from langgraph import Node, Graph, Memory
# Here we create simple Node and Graph classes to simulate the concept.
class BaseNode:
    def __init__(self, name: str):
        self.name = name

    def run(self, memory: dict) -> Any:
...
class ChatbotGraph:
    def __init__(self, nodes: List[BaseNode]):
        self.nodes = nodes
        self.memory: Dict[str, Any] = {}  # This acts as our langgraph memory

    def run(self):
        for node in self.nodes:
            print(f"\n--- Running Node: {node.name} ---")
            node.run(self.memory)
...
# Import MATRIX components (assumed available)
...
def call_groq_llm(prompt: str, model: str, temperature: float, max_tokens: int) -> str:
    """
    Helper function to call Groq's LLM.
    """
    try:
...
def process_with_matrix(current_text: str, previous_responses: List[Dict], context_text: str = "") -> Dict[str, Any]:
    ...
    try:
        ...
        state = matrix.state_encoder.encode_state(
            [],  # dummy history; adjust as needed
            previous_responses,
            current_text
        )
...
# These two functions now store data in the shared memory (mem) rather than globals.
def parse_question_data(question: str, options: list, answer: str, matrix_output: dict, citations: list, mem: dict) -> dict:
    """Parse a single question's data into the structured format and store in memory."""
    if "structured_questions" not in mem:
        mem["structured_questions"] = []
    ...
    mem["structured_questions"].append(question_data)
    return question_data


def store_examination(examination_text: str, selected_option: int, mem: dict):
    """Store examination data in the shared memory."""
    if "examination_history" not in mem:
        mem["examination_history"] = []
    ...
    try:
        ...
        mem["examination_history"].append(examination_entry)
    except Exception as e:
        print(f"Error storing examination: {e}")


# --------------------------------------------------------------------
# QUESTION PROMPTS & INTERVIEW FUNCTIONS
# --------------------------------------------------------------------
    ...
    for question in questions_init:
        while True:
            ...
            if question['type'] in ['MC', 'YN', 'MCM']:
                ...
                if (validated_num := validate_num_input(answer, question['range'])) is not None:
                    responses.append({
                        "question": question['question'],
...
def generate_question_with_options(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Determine the first unused question
        ...
        # If all have been asked, return a fallback question
        return {
...
def get_followup_questions(initial_responses: List[Dict[str, Any]], mem: dict) -> List[Dict[str, Any]]:
    """
    Generate and ask follow-up questions with enhanced processing and citations.
    The structured questions are stored in mem["structured_questions"].
    """
    followup_responses = []
...
Return only the question text.'''
            ...
            question = call_groq_llm(prompt, model="llama-3.3-70b-versatile", temperature=0.3, max_tokens=150)
            
            # Generate options using another Groq call
...
Return each option on a new line (1-4).'''
            ...
            options_response = call_groq_llm(options_prompt, model="llama-3.3-70b-versatile", temperature=0.2, max_tokens=100)
            options = []
            for i, opt in enumerate(options_response.split('\n')):
                if opt.strip():
                    text = opt.strip()
            ...
            while True:
                ...
                if validate_mc_input(answer, options):
                    ...
                    parse_question_data(question, options, answer_text, matrix_output, relevant_docs[-5:], mem)
                    
                    # Display current memory state for debugging
                    print("\nCurrent Structured Questions in Memory:")
                    print(json.dumps(mem.get("structured_questions", []), indent=2))
                    break
...
def get_followup_exams(initial_responses: List[Dict[str, Any]], 
                       followup_responses: List[Dict[str, Any]], mem: dict) -> List[Dict[str, Any]]:
    """
    Generate and recommend examinations with citations.
    The examination history is stored in mem["examination_history"].
    """
    exam_responses = []
    initial_complaint = next((resp['answer'] for resp in initial_responses 
                              if resp['question'] == "Please describe what brings you here today"), "")
...
#:[Fourth possible finding]'''
            ...
            examination_text = call_groq_llm(prompt, model="llama-3.3-70b-versatile", temperature=0.3, max_tokens=200)
            ...
            try:
                ...
                while True:
                    ...
                    if validate_mc_input(answer, options):
                        store_examination(examination_text, int(answer), mem)
                        ...
                        print("\nCurrent Examination History in Memory:")
                        print(json.dumps(mem.get("examination_history", []), indent=2))
                        break
...
List top 3-4 possible diagnoses based on symptoms.'''
        ...
        diagnosis = call_groq_llm(short_diagnosis_prompt, model="llama-3.3-70b-versatile", temperature=0.2, max_tokens=100)
        
        # Treatment recommendations in parts
        treatment_parts = []
        treatment_docs = []
...
Provide 2-3 immediate care steps for {initial_complaint}.'''
            immediate_response = call_groq_llm(immediate_prompt, model="llama-3.3-70b-versatile", temperature=0.2, max_tokens=100)
...
List 2-3 key medications or supplements for {initial_complaint}.'''
            med_response = call_groq_llm(med_prompt, model="llama-3.3-70b-versatile", temperature=0.2, max_tokens=100)
...
List 2-3 home care instructions for {initial_complaint}.'''
            home_response = call_groq_llm(home_prompt, model="llama-3.3-70b-versatile", temperature=0.2, max_tokens=100)
            treatment_parts.append("\nHome Care:\n" + home_response)
        ...
        try:
            ...
            fallback = call_groq_llm(minimal_prompt, model="llama-3.3-70b-versatile", temperature=0.2, max_tokens=50)
            return {
                "diagnosis": fallback,
...
class InitialScreeningNode(BaseNode):
    def __init__(self):
        super().__init__("InitialScreening")

    def run(self, memory: dict) -> Any:
        responses = get_initial_responses()
        memory["initial_responses"] = responses
...
class FollowUpQuestionsNode(BaseNode):
    def __init__(self):
        super().__init__("FollowUpQuestions")

    def run(self, memory: dict) -> Any:
        initial_responses = memory.get("initial_responses", [])
        followup_responses = get_followup_questions(initial_responses, memory)
        memory["followup_responses"] = followup_responses
...
class ExaminationsNode(BaseNode):
    def __init__(self):
        super().__init__("Examinations")

    def run(self, memory: dict) -> Any:
        initial_responses = memory.get("initial_responses", [])
        followup_responses = memory.get("followup_responses", [])
        exam_responses = get_followup_exams(initial_responses, followup_responses, memory)
        memory["exam_responses"] = exam_responses
...
class DiagnosisTreatmentNode(BaseNode):
    def __init__(self):
        super().__init__("DiagnosisTreatment")

    def run(self, memory: dict) -> Any:
        initial_responses = memory.get("initial_responses", [])
        followup_responses = memory.get("followup_responses", [])
        exam_responses = memory.get("exam_responses", [])
        diag_treatment = get_diagnosis_and_treatment(initial_responses, followup_responses, exam_responses)
        memory["diagnosis_treatment"] = diag_treatment
        return diag_treatment
...
class OutputNode(BaseNode):
    ...
    def run(self, memory: dict) -> Any:
        diag_treatment = memory.get("diagnosis_treatment", {})
        print("\nDiagnosis:")
        print("==========")
        print(diag_treatment.get("diagnosis", "No diagnosis available"))
        
        print("\nRecommended Treatment Plan:")
        print("=========================")
        print(diag_treatment.get("treatment", "No treatment plan available"))
        
        print("\nKey References:")
        print("==============")
        seen_sources = set()
        for citation in diag_treatment.get("citations", []):
        ...
        print("==================================")
        if "structured_questions" in memory:
            print(json.dumps(memory["structured_questions"], indent=2))
        
        print("\nFinal Examination History:")
        print("===========================")
        if "examination_history" in memory:
            print(json.dumps(memory["examination_history"], indent=2))
        return diag_treatment
...
def main():
    try:
        ...
        nodes = [
            InitialScreeningNode(),
            FollowUpQuestionsNode(),
            ExaminationsNode(),
            DiagnosisTreatmentNode(),
            OutputNode()
        ]
        
        # Create and run the graph (the shared memory is maintained within the graph)
        chatbot_graph = ChatbotGraph(nodes)
        chatbot_graph.run()
