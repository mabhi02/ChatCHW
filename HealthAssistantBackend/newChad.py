...
def vectorQuotesWithSource(query_embedding: List[float], index, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        promptVal = "Find the page numbers in this text and extract them as a list: " + textVal
        response = groq_client.chat.completions.create(
            messages=[{"role": "system", "content": textVal}],
...
def tprint(text: str, target_language: str) -> None:
    """
    Translates the provided text into the target language and prints it.
    """
...
def main():
    try:
        ...
        # Use tprint to display messages in the target language
...
#:[Fourth possible finding]'''
        ...
        for citation in results["citations"]:
            if citation['source'] not in seen_sources:
                tprint(f"- {citation['source']} (relevance: {citation['score']:.2f})", FINAL_LANGUAGE)
