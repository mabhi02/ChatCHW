...
def vectorQuotesWithSource(query_embedding: List[float], index, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for match in results['matches']:
            ...
            promptVal = "Find the page numbers in this text and extract them as a list. Don't ouput any words as the format should be like just this: '[1,2,3,4,...]'. " + textVal
            response = groq_client.chat.completions.create(
                messages=[{"role": "system", "content": promptVal}],
...
def parse_question_data(question: str, options: list, answer: str, matrix_output: dict, citations: list) -> dict:
    ...
    # Format sources from citations using page numbers
    sources = []
    for cite in citations:
        pages = cite['source'] if isinstance(cite['source'], list) else [str(cite['source'])]
        sources.append(f"Pages: {', '.join(pages)} (relevance: {cite['score']:.2f})")
...
def translate(text: str, target_language: str) -> str:
    """
    Uses a groq completion to translate the provided text into the target language.
    """
...
def main():
    try:
        # Ask for the language to translate to and store as a final variable
        FINAL_LANGUAGE = input("Enter language to translate to: ").strip()
        ...
        # Use tprint to display messages in both English and Hindi
        tprint("\nMedical Assessment Initial Questions", FINAL_LANGUAGE)
        tprint("===================================", FINAL_LANGUAGE)
        
        # Initial Questions Loop
        for question in questions_init:
            while True:
                tprint(f"\n{question['question']}", FINAL_LANGUAGE)
                ...
                if question['type'] in ['MC', 'YN', 'MCM']:
                    ...
                    if question['type'] == 'MCM':
                        tprint("For multiple selections, separate with commas (e.g., 1,2,3)", FINAL_LANGUAGE)
                        if ',' in answer:
                ...
                tprint("Invalid input, please try again.", FINAL_LANGUAGE)

        tprint("\nThank you for providing your information. Here's what we recorded:\n", FINAL_LANGUAGE)
        for resp in initial_responses:
            tprint(f"Q: {resp['question']}", FINAL_LANGUAGE)
        ...
        tprint("\nBased on your responses, I'll ask some follow-up questions.", FINAL_LANGUAGE)
        index = pc.Index("final-asha")
        ...
        while True:
            try:
                ...
                if not relevant_docs:
                    tprint("Error: Could not generate relevant question.", FINAL_LANGUAGE)
                    continue
...
It should not be a compound question like "how long ago did it start and how severe is it" but only one of the 2.
...
For example only describe diarrhea on different levels, not have all of diarrhea, vomiting, cough, and fever as options.
Return each option on a new line (1-4).'''
                ...
                while True:
                    ...
                    if validate_mc_input(answer, options):
                        ...
                        tprint("\nCurrent Structured Questions Array:", FINAL_LANGUAGE)
                        print(json.dumps(structured_questions_array, indent=2))
                        
                        tprint("\nCurrent Examination History:", FINAL_LANGUAGE)
                        print(json.dumps(examination_history, indent=2))
                        break
                ...
                if judge(followup_responses, question):
                    tprint("\nSufficient information gathered. Moving to next phase...", FINAL_LANGUAGE)
                    break
        ...
        tprint("\nBased on your responses, I'll recommend appropriate examinations.", FINAL_LANGUAGE)
        exam_citations = []
...
Previous findings: {str([exam['examination'] for exam in exam_responses]) if exam_responses else "None"}"""
                ...
                if not relevant_docs:
                    tprint("Error: Could not generate relevant examination.", FINAL_LANGUAGE)
                    continue
...
[Examination name]
[Procedure to perform the examination. Make sure this is detailed enough for a medical professional]
...
#:[Fourth possible finding]'''
                ...
                try:
                    ...
                    tprint(f"\nRecommended Examination:", FINAL_LANGUAGE)
                    tprint(examination, FINAL_LANGUAGE)
                    tprint("\nSelect the finding:", FINAL_LANGUAGE)
                    print_options(options)
                    ...
                    tprint(f"Error parsing examination: {e}", FINAL_LANGUAGE)
                    continue
        ...
        # Print diagnosis and treatment using translated output
        tprint("\nDiagnosis:", FINAL_LANGUAGE)
        tprint("==========", FINAL_LANGUAGE)
        tprint(results["diagnosis"], FINAL_LANGUAGE)
        
        tprint("\nRecommended Treatment Plan:", FINAL_LANGUAGE)
        tprint("=========================", FINAL_LANGUAGE)
        tprint(results["treatment"], FINAL_LANGUAGE)
        
        tprint("\nKey References:", FINAL_LANGUAGE)
        tprint("==============", FINAL_LANGUAGE)
        seen_sources = set()
        for citation in results["citations"]:
            pages = citation['source'] if isinstance(citation['source'], list) else [str(citation['source'])]
            pages_str = ", ".join(pages)
            if pages_str not in seen_sources:
                tprint(f"- Pages: {pages_str} (relevance: {citation['score']:.2f})", FINAL_LANGUAGE)
                seen_sources.add(pages_str)
        
        tprint("\nFinal Structured Questions Array:", FINAL_LANGUAGE)
        tprint("==============================", FINAL_LANGUAGE)
        print(json.dumps(structured_questions_array, indent=2))
        
        extract_question_and_options(structured_questions_array, 1)
        
        tprint("\nFinal Examination History:", FINAL_LANGUAGE)
        tprint("========================", FINAL_LANGUAGE)
        print(json.dumps(examination_history, indent=2))
        ...
        tprint("\nAssessment cancelled by user.", FINAL_LANGUAGE)
        sys.exit(0)
    except Exception as e:
